...
func RegisterService(r Registration) error {
	...
	if res.StatusCode != http.StatusOK {
		return fmt.Errorf("failed to register service. Register service responded with code %v",
			res.StatusCode)
	}
...
func (p *providers) Update(pat patch) {
	...
	for _, patchEntry := range pat.Added {
		if _, exists := p.services[patchEntry.Name]; !exists {
	...
	for _, patchEntry := range pat.Removed {
		if providerURLs, exists := p.services[patchEntry.Name]; exists {
...
func (p providers) get(name ServiceName) (string, error) {
	providers, exists := p.services[name]
	if !exists {
